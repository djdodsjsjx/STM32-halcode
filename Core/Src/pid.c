...
void Pid_Control(PID_TypeDef* UPID, double* Temp, double* PIDOut, double* TempSetpoint)
{
	...
	PID(UPID, Temp, PIDOut, TempSetpoint, 0.3, 0.5, 0.1, _PID_P_ON_E, _PID_CD_DIRECT);

	PID_SetMode(UPID, _PID_MODE_AUTOMATIC);
	PID_SetSampleTime(UPID, 500);
	PID_SetOutputLimits(UPID, -150, 150);
    ...
    while (1) 
    {
		*Temp += *PIDOut;
		...
		PID_Compute(UPID);
		DT_SendF1(*UPID->MyInput, *UPID->MySetpoint, 0, 0, 0, 0);
		HAL_Delay(500);		
    }
}
